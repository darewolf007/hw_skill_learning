...
class Robotic_alljoint_FK_Dataset(torch.utils.data.Dataset):
    def __init__(self, data_dict_path, joint_base_xpose, sample_terminal = 10, use_angle = True):
        super(Robotic_FK_Dataset).__init__()
        data_files = os.listdir(data_dict_path)
        self.use_angle = use_angle
        ...
        self.leftfinger_xpos_data = []
        self.rightfinger_xpos_data = []
        self.joint1_xpose_data = []
        self.joint2_xpose_data = []
        self.joint3_xpose_data = []
        self.joint4_xpose_data = []
        self.joint5_xpose_data = []
        self.joint6_xpose_data = []
        self.joint7_xpose_data = []
        self.joint_base_xpose = np.array(joint_base_xpose)
        for file in data_files:
            # if 'noise' in file:
            #     continue
                ...
                for seq_id, all_values in loaded_data.items():
                    for key, value in all_values.items():
                        ...
                        if key == 'rightfinger_xpos':
                            self.rightfinger_xpos_data.extend(value[::sample_terminal])
                        if key == 'body1_xpos':
                            self.joint1_xpose_data.extend(value[::sample_terminal])
                        if key == 'body2_xpos':
                            self.joint2_xpose_data.extend(value[::sample_terminal])
                        if key == 'body3_xpos':
                            self.joint3_xpose_data.extend(value[::sample_terminal])
                        if key == 'body4_xpos':
                            self.joint4_xpose_data.extend(value[::sample_terminal])
                        if key == 'body5_xpos':
                            self.joint5_xpose_data.extend(value[::sample_terminal])
                        if key == 'body6_xpos':
                            self.joint6_xpose_data.extend(value[::sample_terminal])
                        if key == 'body7_xpos':
                            self.joint7_xpose_data.extend(value[::sample_terminal])
        assert len(self.joint_qpose_data) == len(self.end_effector_xpos_data) == len(self.leftfinger_xpos_data) == len(self.rightfinger_xpos_data)
    ...
    def __getitem__(self, index):
        ...
        leftfinger_xpos = self.leftfinger_xpos_data[index]
        rightfinger_xpos = self.rightfinger_xpos_data[index]
        joints_xpose = np.array([self.joint1_xpose_data[index], self.joint2_xpose_data[index], self.joint3_xpose_data[index], self.joint4_xpose_data[index], self.joint5_xpose_data[index], self.joint6_xpose_data[index], self.joint7_xpose_data[index]])
        joint_qpose = torch.from_numpy(joint_qpose).float()
        end_effector_xpos = torch.from_numpy(end_effector_xpos).float()
        leftfinger_xpos = torch.from_numpy(leftfinger_xpos).float()
        rightfinger_xpos = torch.from_numpy(rightfinger_xpos).float()
        joint_base_xpose = torch.from_numpy(self.joint_base_xpose).float()
        joints_xpose = torch.from_numpy(joints_xpose).float()
        return joint_qpose, end_effector_xpos, leftfinger_xpos, rightfinger_xpos, joint_base_xpose, joints_xpose
...
def load_alljoint_data(data_dict_train_path, data_dict_val_path, batch_size_train, batch_size_val, joint_base_xpose = np.array([0,0,0]), sample_terminal = 10):
    train_dataset = Robotic_alljoint_FK_Dataset(data_dict_train_path, joint_base_xpose, sample_terminal= sample_terminal)
    val_dataset = Robotic_alljoint_FK_Dataset(data_dict_val_path, joint_base_xpose,  sample_terminal= 2 * sample_terminal)
    train_dataloader = DataLoader(train_dataset, batch_size=batch_size_train, shuffle=True, pin_memory=True, num_workers=1, prefetch_factor=1)
    val_dataloader = DataLoader(val_dataset, batch_size=batch_size_val, shuffle=False, pin_memory=True, num_workers=1, prefetch_factor=1)
    return train_dataloader, val_dataloader

if __name__ == "__main__":
    load_alljoint_data("/home/haowen/corl2024/data/robotic_model/train", "/home/haowen/corl2024/data/robotic_model/test", 2, 1)